...
public:
    ...
    bool f(int i, int j) {
        ...
        // s1 exhausted → only s2 can be used
        if(i == s1.size()) {
            return dp[i][j] =
                (s2[j] == s3[i+j] && f(i, j+1));
        }

        // s2 exhausted → only s1 can be used
        if(j == s2.size()) {
            return dp[i][j] =
                (s1[i] == s3[i+j] && f(i+1, j));
        }

        // Both are available → try both choices
        return dp[i][j] =
            (s1[i] == s3[i+j] && f(i+1, j)) ||
            (s2[j] == s3[i+j] && f(i, j+1));
    }
...
// // Here DP itself handles all edge cases... Inside main() are not required actually...
